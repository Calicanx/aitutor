...
class DASHSystem:
    ...
    def _load_from_mongodb(self):
        ...
        try:
            ...
            # Load questions from scraped_questions collection (LIMIT for fast startup)
            # Reduced from 2000 to 100 for faster server startup
            questions_docs = list(self.mongo.scraped_questions.find().limit(100))
            self.questions.clear()
            
            log_print(f"[MONGODB] Loading {len(questions_docs)} questions (limited to 100 for fast startup)...")
            
            for i, q_doc in enumerate(questions_docs):
                if i % 100 == 0:
                    log_print(f"[MONGODB] Processing question {i}/{len(questions_docs)}")
                try:
