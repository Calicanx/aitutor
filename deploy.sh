...
# Create temporary substitutions file to handle commas in CORS_ORIGINS
SUBS_FILE=$(mktemp)
cat > "$SUBS_FILE" <<EOF
{
  "_ENV_SUFFIX": "$ENV_SUFFIX_SUB",
  ...
  "_DASH_API_URL": "$DASH_API_URL",
  "_SHERLOCKED_API_URL": "$SHERLOCKED_API_URL",
  "_TEACHING_ASSISTANT_API_URL": "$TEACHING_ASSISTANT_API_URL",
  "_TUTOR_WS": "$TUTOR_WS_URL",
...
gcloud builds submit \
  --config=$CONFIG_FILE \
  --substitutions-from-file="$SUBS_FILE" \
  .
